...
/**
 * Pick the SDP m-section (sdp1 or sdp2) whose SIPREC stream role matches the
 * rtpengine leg being allocated.
 *
 * Early implementations mapped caller→sdp1, callee→sdp2 positionally, which
 * works for SIPREC sources that emit the calling party's media on m-line 1
 * (e.g. BroadWorks `origCall`). It is wrong for sources that emit the called
 * party's media on m-line 1 (e.g. BroadWorks `termCall`, where participant 1
 * is the recorded user and is the callee): the caller leg would then be
 * handed the callee's RTP endpoint, and vice versa, while the recording-slot
 * assignment still followed `mediaStreams[i].role`. The net effect was that
 * each party's audio landed in the other party's mixed-WAV channel, so the
 * transcriber labeled the caller's speech with the callee's phone number.
 *
 * The parser already does the hard work of resolving per-stream roles from
 * the rs-metadata XML (broadWorks call kind, participant AORs, number/aor
 * match, peer inference). We reuse that result here so the leg-to-SDP
 * mapping stays consistent with the leg-to-slot mapping in
 * `resolveRecordingSlots`.
 */
function sdpForLeg(opts, which) {
  const streams = Array.isArray(opts.mediaStreams) ? opts.mediaStreams : [];
  const idx = streams.findIndex((s) => s && s.role === which);
  if (idx === 0) return opts.sdp1;
  if (idx === 1) return opts.sdp2;
  // No role was resolved from SIPREC metadata; preserve the pre-existing
  // positional behaviour so non-BroadWorks payloads keep working as before.
  return which === 'caller' ? opts.sdp1 : opts.sdp2;
}
...
function allocateEndpoint(which, rtpEngine, opts) {
  ...
  const sdp = sdpForLeg(opts, which).replace(/a=inactive\r\n/g, 'a=sendonly\r\n');
...
module.exports.sdpForLeg = sdpForLeg;
module.exports.resolveRecordingSlots = resolveRecordingSlots;
