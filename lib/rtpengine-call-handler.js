...
function buildMetadata(opts, leg) {
  ...
  // rtpengine forwards metadata opaquely; the transcription service consumes
  // these keys to detect 2-channel mixed PCM and to preserve caller/callee
  // channel labels for channel-aware post-processing and diarization.
  kv.push('channels:2');
  kv.push('channel_layout:caller,callee');
  kv.push('channel_0_leg:caller');
  kv.push('channel_1_leg:callee');
