      ...
      try {
        if (typeof result === 'object' && Object.keys(result).length === 1) {
          ...
          (obj[`${prefix}participant`] || []).forEach((p) => {
            ...
            if ((`${prefix}nameID` in p) && Array.isArray(p[`${prefix}nameID`])) {
              ...
              if ('name' in p[`${prefix}nameID`][0] && Array.isArray(p[`${prefix}nameID`][0].name)) {
                ...
                if (typeof name === 'string') partDetails.name = normalizeName(name) ;
                else if (typeof name === 'object') partDetails.name = normalizeName(name._) ;
              }
            }

            if (p.hasOwnProperty(`${prefix}send`)) {
              partDetails.send = p[`${prefix}send`][0];
            }
            if (p.hasOwnProperty(`${prefix}recv`)) {
              partDetails.recv = p[`${prefix}recv`][0];
            }
          ...
          (obj[`${prefix}stream`] || []).forEach((s) => {
            const streamId = getAttr(s, 'stream_id', 'id');
            let sender;
            let senderPart;
            ...
            senderPart.label = s[`${prefix}label`][0];
            ...
            if (isCaller) {
              opts.caller.aor = senderPart.aor ;
              if (senderPart.name) opts.caller.name = senderPart.name;
            }
            else {
              opts.callee.aor = senderPart.aor ;
          ...
          const sessionNode = opts.recordingData[`${prefix}recording`][`${prefix}session`][0];
